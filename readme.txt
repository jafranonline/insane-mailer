...
Fast, queue-powered SMTP and email delivery for WordPress. Send via 20+ providers with logs, retries, and bounce handling.
...
* When an email is sent (either immediately or when the queue is processed). The data transmitted is the email itself: recipient and sender addresses and names, reply-to, subject, message body (HTML and/or plain text), custom headers, and any attachments.
